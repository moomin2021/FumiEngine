...
// 静的メンバ変数の実態
Camera* ParticleManager::sCamera_ = nullptr;
...
void ParticleManager::Draw(uint16_t handle)
{
	// コマンドリスト取得
...
void ParticleManager::CreateVertexBuff()
{
	// デバイス取得
	ID3D12Device* device = DX12Cmd::GetInstance()->GetDevice();
...
void ParticleManager::CreateIndexBuff()
{
	// デバイス取得
	ID3D12Device* device = DX12Cmd::GetInstance()->GetDevice();
